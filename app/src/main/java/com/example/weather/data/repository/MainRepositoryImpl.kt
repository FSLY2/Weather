...
class MainRepositoryImpl @Inject constructor(private val apiProvider: ApiProvider) :
    MainRepository {
    ...
    override fun getCities(): Observable<CitiesDataModel> {
        return apiProvider.provideCityApi().getCities()
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .doOnError { error ->
                Log.e(
                    "MainRepositoryImpl",
                    "getCities: Error getting search cities data",
                    error
                )
            }
    }
